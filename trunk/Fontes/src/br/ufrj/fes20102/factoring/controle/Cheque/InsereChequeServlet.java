package br.ufrj.fes20102.factoring.controle.Cheque;
...
import br.ufrj.fes20102.factoring.modelo.Dominio.CpfInvalido;
...
import br.ufrj.fes20102.factoring.modelo.Persistencia.CpfInvalidoDAO;

/**
 * Servlet responsável por inserir um cheque no Banco de Dados Verifica se o
 * cliente é confiável, se o cheque possui um formato válido, e se o cheque já
 * não está no Banco.
 */
...
public class InsereChequeServlet extends HttpServlet {
	...
	/**
	 * Para inserir um cheque no Banco, é preciso que haja um Cliente associado
	 * ao Cheque. Caso o cliente não conste no Banco de Dados, não é possível
	...
	/**
	 * O número de um cheque é único. Portanto, se já existir um cheque com o
	 * número digitado na entrada pelo usuário, não posso inserí-lo no Banco.
	 * 
	 * @param numero
	 * @return <b>true</b> se o número não for associado a nenhum cheque do
	 *         banco de dados, <br>
	 *         <b>false</b> senão.
	 */
	...
	/**
	 * Se o cheque pode ser inserido, então vai aumentar o número de operações
	 * realizadas com o Cliente associado, portanto, tenho que atualizar o Banco
	...
	/**
	 * Testa se o CPF não está na lista dos não confiáveis (maus pagadores)
	 * 
	 * @param cpf
	 ...
	 * 
	 */
	public boolean validaFormulario(String numero, String cpf, String cnpj,
			String valorBruto, String valorDescontado, String dataVencimento) {

		// Se algum campo estiver vazio
		if (numero.equals("") || cpf.equals("") || cnpj.equals("")
				|| valorBruto.equals("") || valorDescontado.equals("")
	...
	protected void doPost(HttpServletRequest request,
		...
		if (Utilitarios.usuarioLogado(request)) {
			if (validaFormulario(numero, cpf, cnpj, valorBruto,
					valorDescontado, dataVencimento)) {
				// request.setAttribute("cnpjEmpresa", cnpj);
				request.getRequestDispatcher(
						"visao/inserirCheque/insereChequeSucesso.jsp").forward(
						request, response);
			} else {
				// request.getRequestDispatcher("visao/inserirCheque/insereChequeFalha.jsp").forward(request,
				// response);
